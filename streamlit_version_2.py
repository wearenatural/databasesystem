...
def load_data():
    ...
    # --- FIX: Explicitly handle types to prevent PyArrow Crash ---
    # Force Contact_Number to string to handle mix of ints and empty strings
    if 'Contact_Number' in sales_df.columns:
        sales_df['Contact_Number'] = sales_df['Contact_Number'].astype(str)
    
    # Ensure numeric columns are actually numeric where needed for calculations
    cols_to_numeric = ['Quantity_Sold', 'Unit_Selling_Price', 'Total_Sale_Amount', 'Shipping_Cost']
...
if st.session_state.logged_in_user is None:
    ...
    with c2:
        ...
        with st.form("login_form"):
            ...
            if st.form_submit_button("🚀 Launch Portal", type="primary", use_container_width=True): # Changed to use_container_width for button
    ...
    with st.sidebar:
        ...
        if st.button("🚪 Sign Out", use_container_width=True): # Changed to use_container_width
            st.session_state.logged_in_user = None
            st.rerun()
    ...
    if menu == "Dashboard":
        ...
        # 1. KPIs
        # Mobile-friendly: Columns wrap automatically on smaller screens in Streamlit, 
        # but 5 is tight. We keep it standard here.
        ...
        if not df.empty:
            # Type Safety: Ensure calculated columns are numeric
            ...
            if not ah_sales.empty:
                ...
                ah_units_sold = ah_sales['Quantity_Sold'].sum()
        ...
        kpi4.metric("AH Sold (Units)", f"{int(ah_units_sold)}")
        kpi5.metric("Inventory Value", f"₹{total_val:,.0f}")
        ...
        if not df.empty:
            ...
            with r1c1:
                with st.container():
                    ...
                    st.altair_chart(chart, use_container_width=True) # Changed to use_container_width
            ...
            with r1c2:
                with st.container():
                    ...
                    st.altair_chart(channel_chart, use_container_width=True) # Changed to use_container_width
            ...
            with r2c1:
                with st.container():
                    ...
                    st.altair_chart(pie, use_container_width=True) # Changed to use_container_width
            ...
            with r2c2:
                with st.container():
                    ...
                    st.altair_chart(pm_chart, use_container_width=True) # Changed to use_container_width
        else:
    ...
    # --- PAGE 2: NEW SALE ---
        ...
        if 'confirm_sale_state' not in st.session_state:
            st.session_state.confirm_sale_state = False
        ...
        if current_stock_val > 0:
            ...
            if st.button("✅ Confirm Order & Sync", type="primary", use_container_width=True): 
                if not cust_name:
                    ...
                    st.session_state.confirm_sale_state = True
            
            if st.session_state.confirm_sale_state:
                st.markdown("---")
                st.warning("⚠️ **Confirmation:** Are you sure you want to proceed with this order?")
                col_yes, col_no = st.columns(2)
                
                with col_yes:
                    if st.button("✔️ Yes, Place Order", type="primary", use_container_width=True):
    ...
    # --- PAGE 3: SMART INVENTORY ---
        ...
        with tab3:
            ...
            if not hist.empty:
                st.dataframe(hist.sort_values(by="Timestamp", ascending=False), use_container_width=True, height=500) # Changed to use_container_width
            else:
